...
const CommentItem = ({ comment }: TProps) => {
  return (
    <li
      className={classNames(style.comment_item, {
        [style.has_reply]: comment.parentCommentId,
      })}
    >
      ...
      <div className={style.main_wrap}>
        <div className={style.info_wrap}>
          ...
          <Link to={`/postWrote/${comment.writerId}`} className={style.writer}>
            {comment.writerNickname}
          </Link>
          <span className={style.last_update}>
            {getDayMinuteCounter(comment.createdAt)}
          </span>
        </div>
        <div className={style.content}>
          {comment.parentCommentNickname && (
            <span className={style.tag_comment}>
              {comment.parentCommentNickname}
            </span>
          )}
          {comment.content}
